...
function ExamCard({ exam, wishlist, toggleWishlist, owned }) {
    const price = exam.discountPrice ?? exam.price;

    return (
        <article className="glass relative overflow-hidden rounded-2xl transition hover:-translate-y-1">
            {/* Banner */}
            <div className="relative h-40 bg-linear-to-br from-indigo-700 to-violet-900">
                {exam.banner?.url && (
                    <img
                        className="h-full w-full object-cover"
                        src={asset(exam.banner.url)}
                        alt=""
                    />
                )}

                {exam.isFeatured && (
                    <span className="absolute left-3 top-3 rounded-full bg-amber-300 px-2 py-1 text-xs font-bold text-amber-950">
                        <Star size={12} className="mr-1 inline" />
                        Featured
                    </span>
                )}

                {exam.discountPrice != null && !owned && (
                ...
                {/* Wishlist */}
                <button
                    onClick={() => toggleWishlist(exam._id)}
                    className="absolute bottom-3 right-3 rounded-full bg-black/40 p-2 backdrop-blur hover:bg-black/60"
                >
                    <Heart
                        size={22}
                        className={
                            wishlist.includes(exam._id)
            ...
            <div className="p-5">
                <div className="flex gap-3">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-400">{exam.language}</p>
                    </div>
                </div>

                <p className="mt-4 line-clamp-2 text-sm text-slate-400">
                    {exam.description ||
                        "Structured notes, previews and chapter-wise learning."}
                </p>
                ...
                <div className="mt-5 grid grid-cols-2 gap-2">
                    ...
                    {owned ? (
                        <Link
                            className="btn border border-emerald-500 bg-emerald-500/10 text-center text-sm text-emerald-300"
                        ...
                        <Link
                            className="btn border border-white/10 bg-white/5 text-center text-sm"
                            to={`/exam/${exam.slug}`}
                        >
                            Buy Now
                        </Link>
...
function ExamGrid({ limit }) {
    ...
    useEffect(() => {
        api
            .get("/exams", { params: { limit: limit || 100 } })
    ...
    useEffect(() => {
        api
            .get("/wishlist")
            .then((r) =>
                setWishlist((r.data.data.exams || []).map((exam) => exam._id))
            )
            .catch(() => { });
    }, []);

    // Load purchased exams
    useEffect(() => {
        api
            .get("/purchases/my")
            .then((r) => {
                setOwnedExamIds((r.data.data.ownedExamIds || []).map(String));
            })
            .catch(() => { });
    }, []);

    // Wishlist toggle
    const toggleWishlist = async (examId) => {
        try {
            if (wishlist.includes(examId)) {
                await api.delete(`/wishlist/${examId}`);
                setWishlist((prev) => prev.filter((id) => id !== examId));
            } else {
                await api.post(`/wishlist/${examId}`);
                setWishlist((prev) => [...prev, examId]);
            }
        } catch (err) {
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {exams.map((exam) => (
                <ExamCard
                    ...
                    owned={ownedExamIds.includes(String(exam._id))}
                />
            ))}

            {!exams.length && (
                <p className="text-slate-400">No published exams are available yet.</p>
            )}
        </div>
    );
}
...
export function ExamDetail() {
    ...
    const openPdf = async (id, download = false) => {
        try {
            const { data } = await api.get(`/chapters/${id}/access`);

            const pdfUrl = asset(data.data.url);

            // Sample PDF -> Open in new tab
            if (!download) {
                window.open(pdfUrl, "_blank", "noopener");
                return;
            }

            // Download Full PDF -> Download file
            ...
            a.href = blobUrl;
            a.download = "Competition-Notes.pdf";
            ...
            setError(e.response?.data?.message || "PDF unavailable");
        }
    };
    if (error) return <section className="mx-auto max-w-5xl px-5 py-16"><h1 className="text-3xl font-bold">{error}</h1>
        ...
        <div className="relative mx-auto max-w-7xl px-5 py-16">
            ...
            <div className="mt-6 flex flex-wrap items-center gap-4">
                <b className="text-2xl">{money(exam.discountPrice ?? exam.price)}</b>{owned ? <span className="rounded-full bg-emerald-400/20 px-3 py-1 text-emerald-200">Purchased · Full notes unlocked</span> : <button disabled={busy} onClick={buy} className="btn btn-primary"><ShoppingCart className="mr-1 inline" size={16} />{busy ? 'Starting…' : 'Buy now'}</button>}</div></div></section><section className="mx-auto max-w-5xl px-5 py-12"><h2 className="text-2xl font-bold">Subjects & chapters</h2><div className="mt-5 space-y-4">{exam.subjects?.map(subject => <details className="glass rounded-2xl p-5" key={subject._id} open><summary className="cursor-pointer font-bold">{subject.name}<span className="ml-2 text-sm font-normal text-slate-400">{subject.description}</span></summary><div className="mt-4 space-y-3">{subject.chapters?.map(chapter => <article className="rounded-xl bg-white/5 p-4" key={chapter._id}><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold">{chapter.chapterNumber}. {chapter.title}</h3><p className="mt-1 text-sm text-slate-400">{chapter.description}</p></div>{chapter.sampleVideo?.url && <video className="w-full max-w-sm rounded-lg" controls src={asset(chapter.sampleVideo.url)} />}</div><div className="mt-3 flex flex-wrap gap-2"><button onClick={() => openPdf(chapter._id)} className="btn border border-white/10 bg-white/5 text-sm"><FileText className="mr-1 inline" size={15} />Sample PDF</button>{chapter.sampleImages?.map(image => <a key={image.url} className="btn border border-white/10 bg-white/5 text-sm" target="_blank" rel="noreferrer" href={asset(image.url)}>Sample image</a>)}{owned ?
                    ...
                    : chapter.isPremium && <span className="inline-flex items-center px-3 text-sm text-amber-200"><Lock className="mr-1" size={15} />Full notes locked</span>

                }</div></article>)}{!subject.chapters?.length && <p className="text-sm text-slate-400">No published chapters yet.</p>}</div></details>)}{!exam.subjects?.length && <p className="text-slate-400">Subjects will appear here once published.</p>}</div></section></main>
}
